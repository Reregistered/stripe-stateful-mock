...
export namespace customers {
  ...
  export function create(
    ...
    if (
      (params as any).id &&
      accountCustomers.contains(accountId, (params as any).id)
    ) {
    ...
    return expandObject(customer, ["sources", "subscriptions"], params.expand);
  }
  ...
  export function update(
    ...
    services: {
      // TODO: add interface and/or migrate to class
      paymentMethods: any;
    }
  ): Stripe.Customer {
    ...
    if (
      ...
      !customer.sources.data.find(
        (source) => source.id === params.default_source
      )
    ) {
      throw new RestError(400, {
        code: "resource_missing",
    ...
    if (params.invoice_settings?.default_payment_method !== undefined) {
      // get the default payment method
      customer.invoice_settings.default_payment_method =
        services.paymentMethods.retrieve(
          accountId,
  ...
  export function createCard(
    accountId: string,
    customerOrId: string | Stripe.Customer,
    ...
    const customer =
      typeof customerOrId === "object"
        ? customerOrId
        : retrieve(accountId, customerOrId, "customer");
    if (typeof params.source === "string") {
      ...
      throw new Error(
        "Card create options on create customer aren't supported."
      );
    }
  }
  ...
  export function retrieveCard(
    ...
    const card = customer.sources.data.find(
      (card) => card.id === cardId && card.object === "card"
    ) as Stripe.Card;
    if (!card) {
      throw new RestError(404, {
        code: "resource_missing",
  ...
  export function deleteCard(
    ...
    if (customer.default_source === cardId) {
      customer.default_source = customer.sources.data.length
